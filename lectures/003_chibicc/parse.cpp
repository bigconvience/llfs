...
static Node *get_bitor(Token **rest, Token *tok);
static Node *bitxor(Token **rest, Token *tok);
static Node *get_bitand(Token **rest, Token *tok);
...
static Type *declspec(Token **rest, Token *tok, VarAttr *attr) {
  ...
  while (is_typename(tok)) {
    ...
    if (equal(tok, "_Alignas")) {
      ...
      if (is_typename(tok))
        attr->align = get_typename(&tok, tok)->align;
      else
...
static Node *to_assign(Node *binary) {
  ...
  if (binary->lhs->ty->is_atomic) {
    ...
    Obj *new_val = new_lvar("", binary->lhs->ty);
    ...
    cas->cas_new = new_var_node(new_val, tok);
    loop->cond = new_unary(ND_NOT, cas, tok);

    cur = cur->next = loop;
    cur = cur->next = new_unary(ND_EXPR_STMT, new_var_node(new_val, tok), tok);
...
// logand = bitor ("&&" bitor)*
static Node *logand(Token **rest, Token *tok) {
  Node *node = get_bitor(&tok, tok);
  while (equal(tok, "&&")) {
    Token *start = tok;
    node = new_binary(ND_LOGAND, node, get_bitor(&tok, tok->next), start);
  }
  *rest = tok;
  return node;
}
...
static Node *bitxor(Token **rest, Token *tok) {
  Node *node = get_bitand(&tok, tok);
  while (equal(tok, "^")) {
    Token *start = tok;
    node = new_binary(ND_BITXOR, node, get_bitand(&tok, tok->next), start);
  }
  *rest = tok;
  return node;
}
...
static Node *cast(Token **rest, Token *tok) {
  if (equal(tok, "(") && is_typename(tok->next)) {
    Token *start = tok;
    Type *ty = get_typename(&tok, tok->next);
    tok = skip(tok, ")");
...
static Node *primary(Token **rest, Token *tok) {
  ...
  if (equal(tok, "sizeof") && equal(tok->next, "(") && is_typename(tok->next->next)) {
    Type *ty = get_typename(&tok, tok->next->next);
    *rest = skip(tok, ")");
